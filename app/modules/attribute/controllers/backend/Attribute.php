	...
	public function view($page = 1){
		...
		if($catalogueid > 0){
			$config['total_rows'] = $this->Autoload_Model->_condition(array(
				'module' => 'attribute',
				'select' => '`object`.`id`',
				'where' => ((!empty($extend)) ? '`object`.`userid_created` = '.$this->auth['id'].'AND `alanguage` = \''.$this->fclang.'\' ' : 'AND `alanguage` = \''.$this->fclang.'\''),
		...
		if($config['total_rows'] > 0){
			...
			if($catalogueid > 0){
				$data['listattribute'] = $this->Autoload_Model->_condition(array(
					...
					'where' => ((!empty($extend)) ? '`object`.`userid_created` = '.$this->auth['id'].'AND `alanguage` = \''.$this->fc_lang.'\' ' : 'AND `alanguage` = \''.$this->fc_lang.'\''),
					'keyword' => '(`object`.`title` LIKE \'%'.$keyword.'%\' AND `object`.`description` LIKE \'%'.$keyword.'%\')',
					'catalogueid' => $catalogueid,
					'limit' => $perpage,
					'order_by' => '`object`.`catalogueid` desc, `object`.`id` desc',
				));
			}else{
				$data['listattribute'] = $this->Autoload_Model->_get_where(array(
					...
					'keyword' => $keyword,
					'order_by' => ' catalogueid desc, id desc',
				), TRUE);	
			}
