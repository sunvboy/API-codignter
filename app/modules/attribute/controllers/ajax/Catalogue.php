		 ...
		 if(isset($listCatalogue) && is_array($listCatalogue) && count($listCatalogue)){ 
			 foreach($listCatalogue as $key => $val){ 
				$html = $html .'<tr class="gradeX">';
					...
					$html = $html.'<td><a class="maintitle" href="'.site_url('attributes/backend/attributes/view?cataloguesid='.$val['id'].'').'">'.str_repeat('|----', (($val['level'] > 0)?($val['level'] - 1):0)).$val['title'].'</a></td>';
					$html = $html.'<td>';
