    ...
    public function listCatalogue()
    {
        ...
        if ($config['total_rows'] > 0) {
            ...
            $listCatalogue = $this->Autoload_Model->_get_where(array(
                'select' => 'id, title, publish, created, order, level,ishome,highlight,canonical, (SELECT fullname FROM user WHERE user.id = article_catalogue.userid_created) as user_created',
                'table' => 'article_catalogue',
                'where' => array('alanguage' => $this->fclang),
                'keyword' => '(title LIKE \'%' . $keyword . '%\' OR description LIKE \'%' . $keyword . '%\')',
        ...
        if (isset($listCatalogue) && is_array($listCatalogue) && count($listCatalogue)) {
            foreach ($listCatalogue as $key => $val) {
                ...
                $html = $html . '<td><a class="maintitle" style="' . (($val['level'] == 1) ? 'font-weight:600;' : '') . '" href="' . site_url('articles/backend/articles/view?cataloguesid=' . $val['id'] . '') . '" title="">' . (str_repeat('|----', (($val['level'] > 0) ? ($val['level'] - 1) : 0)) . $val['title']) . ' ('.$count.')</a><a href="<?php echo $href ?>" title="Lấy địa chỉ liên kết" onclick="prompt(\'Lấy địa chỉ liên kết\',\''.$href.'\'); return false;"><img border="0" src="template/backend/img/link.png"></a></td>';
                $html = $html . '<td>';
                ...
                $html = $html . '<input type="checkbox" ' . (($val['ishome'] == 1) ? 'checked=""' : '') . ' class="onoffswitch-checkbox publish_frontend" data-module="article_catalogue" data-title="ishome" data-id="' . $val['id'] . '" id="publish_ishome' . $val['id'] . '">';
                ...
                $html = $html . '<div class="switch">';
                $html = $html . '<div class="onoffswitch">';
                $html = $html . '<input type="checkbox" ' . (($val['highlight'] == 1) ? 'checked=""' : '') . ' class="onoffswitch-checkbox publish_frontend" data-module="article_catalogue" data-title="highlight" data-id="' . $val['id'] . '" id="publish_highlight' . $val['id'] . '">';
