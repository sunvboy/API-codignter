...
class Catalogue extends MY_Controller {
	...
	public function view($catalogueid = 0, $page = 1){
        ...
        $config['total_rows'] = $this->Autoload_Model->_get_where(array(
            'select' => 'tb1.id',
            'table' => 'product as tb1',
            'where' => array('tb1.publish_time <=' => $publish_time,'tb1.publish' => 0,'tb1.alanguage' => $this->fc_lang),
            'keyword' => $keyword,
            'join' => $json,
            'query' => $query,
            'distinct' => 'true',
        ...
        if($config['total_rows'] > 0){
            ...
            $productList = $this->Autoload_Model->_get_where(array(
                'distinct' => 'true',
                'select' => 'tb1.customerid,tb1.id, tb1.id as productid, tb1.title, tb1.canonical, tb1.price, tb1.price_sale, tb1.price_contact, tb1.image, tb1.order,tb1.description,tb1.version_json,',
                'table' => 'product as tb1',
                'where' => array('tb1.publish_time <=' => $publish_time,'tb1.publish' => 0,'tb1.alanguage' => $this->fc_lang),
