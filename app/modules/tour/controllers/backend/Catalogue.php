...
class Catalogue extends MY_Controller {
	...
	public function view($page = 1){
		$this->commonbie->permission("tour/backend/catalogue/view", $this->auth['permission']);
		$page = (int)$page;
		$data['from'] = 0;
		$data['to'] = 0;
		...
		if($config['total_rows'] > 0){
			...
			$config['base_url'] = base_url('tour/backend/catalogue/view');
			...
			$data['listCatalogue'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, canonical, image, icon, publish, created, order, level, lft, rgt,ishome,highlight,isaside,isfooter, (SELECT fullname FROM user WHERE user.id = tour_catalogue.userid_created) as user_created',
				'table' => 'tour_catalogue',
				'where' => array( 'alanguage' => $this->fclang),
		...
		$data['script'] = 'tour_catalogue';
		$data['config'] = $config;
		$data['template'] = 'tour/backend/catalogue/view';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Create(){
		$data['script'] = 'tour_catalogue';
		$this->commonbie->permission("tour/backend/catalogue/create", $this->auth['permission']);
		...
		if($this->input->post('create')){
			...
			if($this->form_validation->run($this)){
				...
				if($resultid > 0){
					...
					$this->session->set_flashdata('message-success', 'Thêm loại tour mới thành công');
					redirect('tour/backend/catalogue/view');
				}
			}
		}
		
		$data['template'] = 'tour/backend/catalogue/create';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Update($id = 0){
		$data['script'] = 'tour_catalogue';
		$this->commonbie->permission("tour/backend/catalogue/update", $this->auth['permission']);
		$id = (int)$id;
			...
			if($this->form_validation->run($this)){
				...
				if($flag > 0){
					...
					$this->Autoload_Model->_delete(array(
						'where' => array('module' => 'tour_brand', 'catalogueid' => $id),
					...
					$this->nestedsetbie->Recursive(0, $this->nestedsetbie->Set());
					$this->nestedsetbie->Action();
					$this->session->set_flashdata('message-success', 'Cập nhật loại tour thành công');
					redirect('tour/backend/catalogue/view');
				}
			}
		}
		...
		$data['template'] = 'tour/backend/catalogue/update';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
