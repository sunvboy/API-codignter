...
class Acatalogue extends MY_Controller {
	...
	public function view($page = 1){
		$this->commonbie->permission("tour/backend/acatalogue/view", $this->auth['permission']);
		$page = (int)$page;
		$data['from'] = 0;
		$data['to'] = 0;
		...
		$config['total_rows'] = $this->Autoload_Model->_get_where(array(
			...
			'where' => array('tour' => 1,'alanguage' => $this->fclang),
			'keyword' => $keyword,
			'count' => TRUE,
		));
		if($config['total_rows'] > 0){
			$this->load->library('pagination');
			$config['base_url'] = base_url('attribute/backend/acatalogue/view');
			...
			$data['listCatalogue'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title,  publish, created, order,car,room,tour,level,ishome,highlight, (SELECT fullname FROM user WHERE user.id = attribute_catalogue.userid_created) as user_created',
				'table' => 'attribute_catalogue',
				'where' => array('tour' => 1,'alanguage' => $this->fclang),
		...
		$data['config'] = $config;
		$data['template'] = 'tour/backend/acatalogue/view';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Create(){
		$data['script'] = 'attribute_catalogue';
		$this->commonbie->permission("tour/backend/acatalogue/create", $this->auth['permission']);
		if($this->input->post('create')){
			...
			if($this->form_validation->run($this)){
				...
				if($resultid > 0){
					...
					redirect('tour/backend/acatalogue/view');
				}
			}
		}
		$data['template'] = 'tour/backend/acatalogue/create';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Update($id = 0){
		$data['script'] = 'attribute_catalogue';
		$this->commonbie->permission("tour/backend/acatalogue/update", $this->auth['permission']);
		$id = (int)$id;
		...
		$data['template'] = 'tour/backend/acatalogue/update';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
}
