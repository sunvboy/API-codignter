    ...
    public function listCatalogue()
    {
        ...
        if ($config['total_rows'] > 0) {
            ...
            $listCatalogue = $this->Autoload_Model->_get_where(array(
                'select' => 'id, title, publish, created, order, level,highlight,ishome,canonical, (SELECT fullname FROM user WHERE user.id = tour_catalogue.userid_created) as user_created',
                'table' => 'tour_catalogue',
                'where' => array('alanguage' => $this->fclang),
                'keyword' => '(title LIKE \'%' . $keyword . '%\' OR description LIKE \'%' . $keyword . '%\')',
        ...
        if (isset($listCatalogue) && is_array($listCatalogue) && count($listCatalogue)) {
            foreach ($listCatalogue as $key => $val) {
                ...
                $html = $html . '<td><a class="maintitle" style="' . (($val['level'] == 1) ? 'font-weight:600;' : '') . '" href="' . site_url('tour/backend/tour/view?cataloguesid=' . $val['id'] . '') . '" title="">' . (str_repeat('|----', (($val['level'] > 0) ? ($val['level'] - 1) : 0)) . $val['title']) . ' ('.$count.')</a>															
<a href="'.$href.'" title="Lấy địa chỉ liên kết" onclick="prompt(\'Lấy địa chỉ liên kết\',\''.$val['canonical'].'.html\'); return false;"><img border="0" src="template/backend/img/link.png"></a>
                ...
                $html = $html . '<input type="checkbox" ' . (($val['ishome'] == 1) ? 'checked=""' : '') . ' class="onoffswitch-checkbox publish_frontend" data-id="' . $val['id'] . '" data-module="tour_catalogue" data-title="ishome" id="publish_ishome' . $val['id'] . '">';
                ...
                $html = $html . '<div class="switch">';
                $html = $html . '<div class="onoffswitch">';
                $html = $html . '<input type="checkbox" ' . (($val['highlight'] == 1) ? 'checked=""' : '') . ' class="onoffswitch-checkbox publish_frontend" data-id="' . $val['id'] . '" data-module="tour_catalogue" data-title="highlight" id="publish_highlight' . $val['id'] . '">';
                ...
                $html = $html . '<a type="button" href="' . (site_url('tour/backend/catalogue/update/' . $val['id'] . '')) . '" class="btn btn-sm btn-primary mr5"><i class="fa fa-edit"></i></a>';
                $html = $html . '<a type="button" class="btn btn-sm btn-danger ajax-delete" data-title="Lưu ý: Khi bạn xóa danh mục, toàn bộ tour trong nhóm này sẽ bị xóa. Hãy chắc chắn bạn muốn thực hiện chức năng này!" data-id="' . $val['id'] . '" data-module="tour_catalogue"><i class="fa fa-trash"></i></a>';
                $html = $html . '</td>';
                $html = $html . '</tr>';
            }
        } else {
