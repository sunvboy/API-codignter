	...
	public function listCatalogue(){
		...
		if($config['total_rows'] > 0){
			...
			$listCatalogue = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, publish, created, order, level,highlight,ishome, (SELECT fullname FROM user WHERE user.id = page_catalogue.userid_created) as user_created',
				'table' => 'page_catalogue',
				'where' => array('alanguage' => $this->fclang),
				'keyword' => '(title LIKE \'%'.$keyword.'%\' OR description LIKE \'%'.$keyword.'%\')',
		 ...
		 if(isset($listCatalogue) && is_array($listCatalogue) && count($listCatalogue)){ 
			 foreach($listCatalogue as $key => $val){ 
				$html = $html .'<tr class="gradeX">';
                 ...
                 $html = $html.'<input type="checkbox" '.(($val['ishome'] == 1) ? 'checked=""' : '').' class="onoffswitch-checkbox publish_frontend" data-module="page_catalogue" data-title="ishome" data-id="'.$val['id'].'" id="publishishome'.$val['id'].'">';
                 ...
                 $html = $html.'<div class="switch">';
                 $html = $html.'<div class="onoffswitch">';
                 $html = $html.'<input type="checkbox" '.(($val['highlight'] == 1) ? 'checked=""' : '').' class="onoffswitch-checkbox publish_frontend" data-module="page_catalogue" data-title="highlight" data-id="'.$val['id'].'" id="publishhighlight'.$val['id'].'">';
                 $html = $html.'<label class="onoffswitch-label" for="publishhighlight'.$val['id'].'">';
                 $html = $html.'<span class="onoffswitch-inner"></span>';
                 $html = $html.'<span class="onoffswitch-switch"></span>';
                 $html = $html.'</label>';
                 $html = $html.'</div>';
